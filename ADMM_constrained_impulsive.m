function [ x,k,SNR,PSNR,SIM,tg,PSNR2]= ADMM_constrained_impulsive(f_true,b,psf,iter,epsilon,mu,known,rho)
...
% mu --------------------- regularization parameter of total variation
...
while k <= iter
    ...
    x_update = x - lambda*( opDadj(lam1) + rho*opDadj(opD(x)-y) + lam2 + rho*(x-z) + lam3 + rho*(x - w) + A_adj(lam4)  + rho*(A_adj(A_dir(x)-q)) );
    
    yp = opD(x_update) + 1/rho * lam1;
   y_update = max(abs(yp)-1/rho,0).*sign(yp);  % ATV
%   yp_sum = sqrt(yp(:,:,1).^2 + yp(:,:,2).^2);
%     yp_sum(yp_sum==0) = 1;  
%   y_update(:,:,1) = (max(yp_sum - 1/rho,0)./yp_sum).*yp(:,:,1);
%   y_update(:,:,2) = (max(yp_sum - 1/rho,0)./yp_sum).*yp(:,:,2);
  
    zp = x_update + 1/rho * lam2;
    z_update = proj_bound(zp,0,1);
    
    wp = x_update + 1/rho * lam3;
    w_update = Prox_lambda_nuclear_norm(wp,mu/rho);
   ...
   lam3_update = lam3 + rho*(x_update - w_update);
