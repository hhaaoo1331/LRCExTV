function [ x,k,SNR,PSNR,SIM,tg,PSNR2]= Dong_ADMM_constrained_impulsive(f_true,b,psf,iter,epsilon,known,rho)
...
while k <= iter
    ...
    x_update = x - lambda*( opDadj(lam1) + rho*opDadj(opD(x)-y) + A_adj(lam4)  + rho*(A_adj(A_dir(x)-q)) );
